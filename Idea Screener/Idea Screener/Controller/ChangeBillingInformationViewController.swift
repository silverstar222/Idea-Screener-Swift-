//
//  ChangeBillingInformationViewController.swift
//  Idea Screener
//
...
class ChangeBillingInformationViewController: UIViewController, UITextFieldDelegate {
        ...
        if user.isCompleted {
            ...
            STPAPIClient.shared().createToken(withCard: cardParams) { [unowned self] (token: STPToken?, error: Error?) in
                ...
                self.postExternalStripeToken(token: self.stripeToken, complete: { (result) in
                    if result {
                        let alert = UIAlertController(title: nil, message: "Billing information has been changed!", preferredStyle: .alert)
            ...
            let alert = UIAlertController(title: "Error", message: "You have to become a respondent first!", preferredStyle: .alert)
            let alertAction = UIAlertAction(title: "OK", style: .default, handler: { (_) in
                sender.isEnabled = true
                self.hideIndicator()
                self.navigationController?.popViewController(animated: true)
